...
- **Stable resolution by name** — `service-id` values are per-run: the host mints a fresh `svc-N` on every `register-service` call, so callers that persist a target across restarts persist the `(plugin-id, service-name)` pair and re-resolve on demand via `host-services::resolve-service`. The name matches `capabilities.declares_services` in the callee's manifest, which is fixed for the plugin's lifetime.
- **Same-instance dispatch is not supported** — the callee's supervisor drives exactly one wasm `Store`; a dispatch to a service the caller's own instance owns would queue an `ExecuteService` to a supervisor already parked awaiting the caller's reply, which is deadlock-by-construction. The dispatcher rejects the caller==target case up-front with `Error::InvalidArgument` and the message `same-instance dispatch is not supported`; plugins that colocate services in one instance dispatch between them in plugin-local code without going through `call-service`.
- **Cycles** — the dispatcher carries the in-flight call chain across the task hop (in `ControlCommand::ExecuteService`); the callee's supervisor re-scopes a `tokio::task_local` on its task before driving the wasm so any nested `call-service` sees the full chain. Multi-hop A→B→…→A calls are rejected with `Error::InvalidArgument` and the message `cycle detected` — distinct from the same-instance case so operators can tell the two apart in logs.
