...
- **Stable resolution by name** — `service-id` values are per-run: the host mints a fresh `svc-N` on every `register-service` call, so callers that persist a target across restarts persist the three-tuple `(plugin-id, instance-id, service-name)` and re-resolve on demand via `host-services::resolve-service`. All three parts are stable: `plugin-id` from the callee's manifest, `instance-id` operator-supplied to `Engine::start_instance` (already the per-instance KV/blob storage key), `service-name` a `capabilities.declares_services` entry. The three-tuple is required — same-plugin multi-instance registrations share the first two, and `(plugin_id, name)` alone is not unique.
- **Caller-side capability gate (H10)** — before routing, the dispatcher checks that the target service's owning `plugin.id` appears in the *caller's* `[capabilities] consumes_services` grant. Without it, `call-service` returns `Error::PermissionDenied` before `acquire_call` runs, so a refused call spends no refcount and cannot influence `remove-service` timing. `resolve-service` is unauth by design (the id is discoverable, the invocation is not); the grant is the sole authorization surface for cross-plugin dispatch, and can be narrowed at install time via `plugin_installation.granted_capabilities_json`.
