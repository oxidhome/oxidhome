...
2. **Device registry + host-owned state projection (H9)** — the registry is the canonical list of devices (IDs, names, capability specs). Host-minted device ids are deterministic: `dev-<hex(SHA-256(installation_uuid, instance_id, local_id))>`; a plugin's re-registration on restart resurrects the same id, a reinstall (fresh `installation_uuid`) mints a new one. **Current state lives in a separate host-owned projection** (`DeviceStateStore`), keyed on `(device_id, capability)` — the registry never stored state past registration-time `initial_state` (review report H2). The projection is updated via two distinct operations: `replace_snapshot` (full-state input — used by `register-device` `initial_state` and `execute-command` `OkWithState` returns) overwrites the slot; `apply_delta` (partial-changes input — used by `state-changed` events under the same `publish_sequence` gate as `event_log.record`) merges by key so a color-light update reporting only `hue` composes with the last-known `saturation` / `value` / `color-temp-kelvin`. Choosing the wrong operation matters: pre-fix, register-device fell through the merge path so a re-register that omitted a field left the stale value present — the documented remove-and-re-register deletion procedure didn't actually delete. `OkWithState` also re-validates ownership + capability declaration after the guest returns before projecting, so a plugin that calls `remove-device` mid-`execute-command` and then returns `OkWithState` can't resurrect the removed slot. Each entry carries a store-wide `global_revision` (the sole ordering axis — a per-`(device, capability)` counter was removed in round 9 because preserving it across stale-cap eviction would either grow the store unboundedly with tombstones or force a global epoch rotation on every eviction, a `DoS` vector), host `received_ms` and plugin-supplied `observed_ms` (trust-separated, same shape as `event_log`), the supervisor `source_generation` at write time, and a `quality` marker (`fresh`/`stale`). Revision allocation and insertion linearize under one lock, and snapshot/delta reads take the same lock — a reader observing `current_revision() = N` sees every entry with `global_revision ≤ N`, and the API returns `(revision, entries)` atomically. Every response also carries an opaque `epoch` (128-bit OS-random nonce, minted at store construction) so a client can detect a daemon restart independent of the revision counter, and `deltas_since` returns `reset_required = true` on either an above-current cursor (restart) or a below-eviction-watermark cursor (missed a stale flip). Lifecycle events reconcile the projection: `remove-device` flips the device's entries to `Stale`; `update-device` with a narrower `capabilities` list flips the dropped capabilities to `Stale`; `register-device` runs the atomic `reset_and_seed_device` composite — flips every pre-existing `Fresh` entry for the stable id to `Stale` and re-seeds the current registration's `initial_state`, all under one lock, so a re-register that keeps a capability but omits its initial state doesn't silently retain the pre-restart value. When the owning instance stops (or a new supervisor life begins), the supervisor calls the composite `restart_generation` — mark-stale + bump-generation under one store write lock — so no delayed writer can slip a fresh entry stamped with the pre-restart generation past the sweep. **The projection is a materialized-latest view, not an append-only stream**: a slot updated multiple times between polls has its intermediate `global_revision` values overwritten in place, so `state/changes` responses reflect the current per-slot value only. Callers who need the full history read the durable `event_log` (`GET /api/v1/events`). Snapshot API: `GET /api/v1/devices/{id}/state` returns `{revision, capabilities}` for one device; `GET /api/v1/devices/state?cursor=X&limit=N` returns a paginated full-store snapshot as `{revision, devices[], next_cursor?}` — the resync primitive a client falls back on after `reset_required`, so a `devices:read`-only client can recover without also holding `devices:list`. Pagination is by *device* (all capabilities for a given device land on the same page, so per-device snapshots stay atomic) and by *server-issued opaque cursor* that encodes epoch + pinned revision + `after_device_id`. First-page requests carry no cursor; the response returns the store's current revision as both `revision` and (embedded in) `next_cursor`. Continuation requests pass the previous `next_cursor` back verbatim; the server decodes it, verifies its epoch still matches the store's current epoch (409 if not — daemon restart mid-resync, restart from scratch), and echoes the pinned revision as `revision`. After the final page, the client uses that pinned revision as its `since_revision` for `/state/changes`, so writes that landed mid-pagination (including to devices already paged past or new devices sorting behind the cursor) — all of which carry `global_revision > pinned_revision` by construction — surface on the next cursor poll. Because the cursor is server-issued and epoch-verified, the client can neither omit the pin on continuation nor forge a stale one. The store's entries live in a `BTreeMap` keyed on `(device_id, capability)`, so each page's range scan is `O(log N + k · caps-per-device)` — no full-store copy per request, no lock hold beyond the returned entries. Cursor catch-up: `GET /api/v1/devices/state/changes?since_revision=N&limit=M` returns entries with `global_revision > N`, ascending. Each `(device, capability)` slot is bounded to `MAX_FIELDS_PER_SLOT` (64) distinct field keys — `apply_delta` accepts updates to existing keys freely but drops new keys past the cap, and `replace_snapshot` / `reset_and_seed_device` truncate oversized inputs — so a plugin repeatedly publishing `state-change` events with unique keys can't grow one Fresh slot without bound (that path bypasses both the per-instance device quota — only one slot — and the stale-eviction cap — never flipped stale). All three are gated on the `devices:read` scope (distinct from `devices:list` which reads the registration snapshot, and `devices:command` which drives actuation). `register-device` enforces a per-instance quota (`MAX_DEVICES_PER_INSTANCE`, currently 1024) so a plugin looping unique `local_id`s can't grow the registry (and the projection whose `Fresh` entries never evict) without bound, a per-device capability cap (`MAX_CAPABILITIES_PER_DEVICE = 32`) so one device can't grow its projection footprint via unique `extension(<name>)` entries, and rejects `DeviceInfo` with duplicate capability specs or duplicate `initial-state` entries so two conflicting values for the same capability can't silently overwrite each other on the projection's canonical slot. Each `(device, capability)` slot is bounded to `MAX_FIELDS_PER_SLOT` (64) distinct field keys AND `MAX_BYTES_PER_SLOT` (64 KiB) of merged value bytes, both enforced at the WIT boundary: `publish_event` for `state-changed`, `execute_command` for `OkWithState`, and `register-device` / `update-device` for `initial_state` all pre-check via `DeviceStateStore::check_delta_admission` / `check_snapshot_admission` and return `WitError::InvalidArgument` before persisting to the durable event log — pre-fix, the store silently truncated overflow while the event log and bus fanout carried the full state, producing two conflicting authoritative views. Manifest validation caps `capabilities.declares_devices` at `MAX_DECLARES_DEVICES = 64` so plugin-declared capability lists can't push per-register-device admission work into DoS territory. The paginated recovery cursor is HMAC-SHA256 signed with a per-process 256-bit secret (`<epoch>.<revision>.<device>.<hmac128>`); a client can neither forge a cursor from scratch (the epoch is public but the secret isn't) nor mutate the pinned revision or device id in an existing one, so the pin protocol's cross-page consistency guarantee holds even against a malicious client. `verify_cursor` compares the epoch before validating the MAC so a legitimate cursor from a prior daemon life (rotated secret AND rotated epoch) surfaces as `EpochChanged` → 409 (restart the resync), not `Bad` → 400 (client bug). Admission for `apply_delta` mirrors the store's write-path merge-vs-replace rule (only baselines against the existing slot when it's Fresh and same-generation, else treats the incoming vec as a replace) and normalizes duplicate keys in the incoming vec to last-wins before measuring — so a post-restart delta doesn't get rejected against a stale slot's fields, and a `[a="", a=<big>]` sequence isn't undercounted by the byte cap. Snapshot inputs (`register-device` `initial_state`, execute-command `OkWithState`) also canonicalize duplicates last-wins before storing so the projection, wire response, and REST HashMap conversion don't disagree about which value is authoritative. A per-instance aggregate quota (`MAX_PROJECTED_BYTES_PER_INSTANCE = 16 MiB`) checked at admission bounds one plugin's total projected footprint regardless of how it's split across devices and capabilities; register-device runs the aggregate admission BEFORE mutating the registry (the id is deterministic, so it can be computed up front), so a rejection can't destroy pre-existing metadata via a rollback that overwrites-then-deletes. The aggregate excludes `Stale` entries — the stale-cap evictor bounds their contribution separately, and charging Stale bytes against the live quota would let one instance fill 16 MiB, mark it Stale, and lock itself out of publishing new state until the total-store stale sweep fires. `execute-command` `OkWithState` is canonicalized once (last-wins on duplicate keys) and the canonical vec feeds both the projection write AND the returned `CommandResult`, so the projection, REST HashMap conversion, and Connect RPC duplicate-preserving encoding all agree on the authoritative values. The full-store snapshot endpoint truncates each page at both a device-count cap (default 64, max 256) and a cumulative-page-byte cap (1 MiB) at device boundaries, so one request never allocates more than the ceiling regardless of how large per-device slots are. The caps are applied on the raw `Arc<DeviceState>` entries fetched from the store (cheap ref-clone), each page accumulates a *whole device* into a pending buffer and commits it as one unit at the device boundary (so a page can't silently exceed the ceiling by up to a device's capability contribution), and only the committed entries are deep-cloned into wire values — pre-fix, `DeviceStateEntry::from_state` ran on every fetched entry (up to ~128 MiB at max limit) before the byte cap truncated the response to 1 MiB. `execute-command` `OkWithState` runs the per-slot admission on borrowed data (`projected_slot_size` uses a `HashMap<&str, usize>` bounded by unique-key count, early-exits on field-count overflow — the count is monotonic — and checks bytes only after processing every duplicate, so a `[big, small]` last-wins sequence for the same key is accepted like the write path would) before the ownership/capability check, consumes the guest-returned vec (via `match` on `CommandResult`), and canonicalizes by *moving* values through the dedup pipeline (`canonicalize_owned`) — no per-`KeyValue` clone on either the reject or the accept path.
...
8. **UI / API surface** — HTTP endpoints for clients to consume, currently split across a JSON/REST + WebSocket surface and a Connect RPC surface (which speaks Connect, gRPC, and gRPC-Web off the same handlers) on one shared listener. Plugin UIs (C6) load through a two-step ticketed flow that works in real browsers. **The sandboxed iframe + typed postMessage broker is the sole path any plugin-authored UI code executes on** — Phase 13 must not offer a mode where plugin JavaScript loads directly into the shell realm (e.g. as a Web Component imported into the shell's DOM), because that would give plugin code the shell's origin, the shell's DOM, the operator's bearer, and every capability the shell holds. CSP alone is not isolation; opaque-origin iframes are. Any future extension-point contract (config panels, dashboard widgets, custom detail pages) must be defined as a message envelope on the per-plugin `MessagePort`, not as a Web-Component import surface: (1) the dashboard (which holds the bearer) `POST`s `/api/v1/plugins/{plugin_id}/ui-session` (gated on `plugins:ui`) and reads back `{url, expires_ms}` where `url` is `/api/v1/plugins/{plugin_id}/ui?tk=<opaque>`; (2) the dashboard hands that URL to the browser as an iframe `src` (or a `window.open` argument) — no `Authorization` header needed, because browsers don't propagate it to iframe subresources or top-level document navigations. `/ui` verifies the ticket and returns a wrapper page whose only body is an `<iframe sandbox="allow-scripts allow-forms">` — deliberately no `allow-same-origin`, so the browser assigns each iframe a fresh opaque origin, distinct from the daemon's and from every other plugin's. The wrapper embeds the same ticket into the inner iframe's `src="/api/v1/plugins/{plugin_id}/ui/frame?tk=<opaque>"`; `/ui/frame` verifies the ticket a second time. Both `/ui` and `/ui/frame` sit on a router *outside* the bearer middleware — the ticket is the only auth check that reaches them. Tickets are HMAC-SHA256-signed with a per-process 256-bit secret (rotated on daemon restart), 5-minute TTL, bound to both `plugin_id` and the current `installation_uuid` (so an uninstall + reinstall under the same id mints a new UUID and old tickets fail closed as 404 — same shape as "no such plugin", no enumeration oracle). Raw ticket length is capped both at the axum extractor layer (a custom `TicketQuery` that reads `parts.uri.query()` as a borrowed slice and rejects anything longer than `MAX_TICKET_LEN + "tk=".len()` before allocating the owned value) and inside the HMAC verifier (`rsplit_once` + `splitn(3, '~')`, never collects into a `Vec`), so the public `/ui/frame` verifier can't be pushed into per-request allocation growth by a huge `tk=…` value up to hyper's request-head limit. Both public endpoints verify the ticket's HMAC *before* touching the installed-plugin registry — so `?tk=garbage` returns 400 uniformly regardless of whether the URL path names an installed plugin, and even a MAC-valid but wrong-target ticket surfaces as 404 (same shape as "no such plugin"), giving unauthenticated callers no oracle to enumerate installed integrations. Every response on the three UI endpoints — success bodies, ticket errors, scope 403s excluded, 404s and 501s included — carries `Cache-Control: no-store` so a shared proxy can't replay a stored response past ticket expiry or through an uninstall+reinstall. The wrapper's inline broker JS establishes a fresh `MessageChannel` per iframe and transfers `port2` to it, so all subsequent host↔plugin IPC flows on a dedicated port — an origin-based `postMessage` filter would be useless here (opaque-origin senders all report `origin: "null"`), and the per-port binding makes cross-plugin dispatch impossible by construction. `plugin_id` is percent-encoded into the URL and HTML-escaped in every template slot to defend against a future validator relaxation that admits URL / HTML syntax. Phase 13 adds three surfaces on top of C6's substrate: (a) the `[ui]` block on the plugin manifest with validation on every asset path (same escape family as `runtime.wasm`), widget-count / duplicate checks, and a fixed enum on the three `[ui.permissions]` fields that map to CSP directives; (b) `GET /api/v1/plugins/{plugin_id}/schema` returns the manifest's `config` + `[ui]` verbatim so the shell can render declarative config forms and locate declared UI assets in one round trip (gated on `plugins:list`); (c) persistent user-composed dashboards in a new `dashboard` table (migration 15), exposed as five REST endpoints under `/api/v1/dashboards` (list / GET / POST / PUT / DELETE) gated on `dashboards:read` / `dashboards:write`. Dashboard layout is opaque bytes to the host — the shell owns the JSON tree shape, versioned by `schema_version` so a widget-catalog change can drive a shell-side migration. Cross-owner reads / writes / deletes return 404 so the multi-user follow-up doesn't leak the id-space to other operators through the status code.
...
- **Stable, immutable resolution (H10)** — `service-id` values are per-run: the host mints a fresh `svc-N` on every `register-service` call, so callers that persist a target across restarts persist the three-tuple `(plugin-id, instance-id, service-local-id)` and re-resolve on demand via `host-services::resolve-service`. All three components are stable. The address uses the callee's **immutable `local-id`** (the plugin's own bookkeeping key from `service-info.local-id`), not the human-readable `name` — `register-service` refuses a duplicate `(instance, local-id)` and `update-service` refuses any change to `local-id`, so a persisted address cannot silently invalidate under a rename.
- **Caller-side capability gate (H10)** — before routing, the dispatcher matches the target service's `(owner_plugin_id, owner_instance, local_id)`, the requested command name, **and the actual caller's instance-id** against the caller's structured `[capabilities] consumes_services` grants. Each grant is a resource selector `{plugin, instance, service, commands, caller_instance}`; `instance`, each entry in `commands`, and `caller_instance` may be `"*"` to match any target instance / any command / any caller instance. Because the authorization keys off the immutable `local-id`, a callee that renames its service via `update-service` cannot silently bypass or shadow a grant. The `caller_instance` axis is what stops a grant on one instance of a multi-instance caller plugin from authorizing every other instance of the same plugin — plugin manifests typically leave it `"*"` and operators narrow it in the granted copy. `"*"` is reserved as the wildcard sentinel at identifier boundaries: `Engine::start_instance` refuses `"*"` as an instance-id, and `register-service` refuses `"*"` as a command name, so a real identifier can never collide with the wildcard.
- **Grant application (requested ∧ granted, no cross-product)** — a call is authorized iff at least one entry in the plugin-declared **requested** list matches AND at least one entry in the operator-narrowed **granted** list matches (H10 round-4). Both checks run at call time against the target-tuple; no materialized intersection is stored. This is semantically equivalent to requested ∩ granted but avoids the O(N²) blowup a cross-product would produce when a manifest declares N wildcard-target selectors × N wildcard-caller selectors (each pair produces a distinct narrowed record). Manifest validation caps `consumes_services` at [`MAX_CONSUMES_SERVICES_GRANTS`] entries with each grant's `commands` capped at [`MAX_COMMANDS_PER_GRANT`], so the per-call check is bounded per instance. `effective_capabilities` carries only the granted list on the intersected `CapabilitiesSection`; the requested list rides separately on `PluginState`.
- **Same-instance dispatch is not supported** — the callee's supervisor drives exactly one wasm `Store`; a dispatch to a service the caller's own instance owns would queue an `ExecuteService` to a supervisor already parked awaiting the caller's reply, deadlock-by-construction. The dispatcher rejects the `caller_instance == target_instance` case up-front with `Error::InvalidArgument` and the message `same-instance dispatch is not supported`; plugins that colocate multiple services in one instance dispatch between them in plugin-local code without going through `call-service`.
- **Cycles** — the dispatcher carries the in-flight call chain across the task hop (in `ControlCommand::ExecuteService`); the callee's supervisor re-scopes a `tokio::task_local` on its task before driving the wasm so any nested `call-service` sees the full chain. Multi-hop A→B→…→A calls are rejected with `Error::InvalidArgument` and the message `cycle detected` — distinct from the same-instance case so operators can tell them apart in logs.
