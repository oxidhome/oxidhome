...
- **Caller-side capability gate (H10)** — before routing, the dispatcher matches the target service's `(owner_plugin_id, owner_instance, local_id)`, the requested command name, **and the actual caller's instance-id** against the caller's structured `[capabilities] consumes_services` grants. Each grant is a resource selector `{plugin, instance, service, commands, caller_instance}`; `instance`, each entry in `commands`, and `caller_instance` may be `"*"` to match any target instance / any command / any caller instance. A call is authorized when at least one grant matches all five axes. Because the authorization keys off the immutable `local-id`, a callee that renames its service via `update-service` cannot silently bypass or shadow a grant. The `caller_instance` axis (H10 round-3 finding 2) is what stops a grant on one instance of a multi-instance caller plugin from authorizing every other instance of the same plugin — plugin manifests typically leave it `"*"` and operators narrow it in the granted copy. `"*"` is reserved as the wildcard sentinel at identifier boundaries: `Engine::start_instance` refuses `"*"` as an instance-id, and `register-service` refuses `"*"` as a command name, so a real identifier can never collide with the wildcard.
- **Grant intersection (requested ∩ granted)** — `plugin_installation.granted_capabilities_json` intersects `consumes_services` **semantically**, not by whole-record equality: a plugin requesting `{instance="*", commands=["*"]}` narrowed by an operator's `{instance="foo", commands=["get"]}` produces `{instance="foo", commands=["get"]}` in the effective grant. `instance` / `caller_instance` intersect as `"*" ∩ X = X`; `commands` intersect as sets (with `"*"` treated as the universal set). Non-overlapping requested × granted pairs drop out entirely. This is what makes narrowing usable — the pre-fix whole-record intersection turned every wildcard-narrowing attempt into denial (H10 round-3 finding 1).
