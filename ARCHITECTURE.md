...
2. **Device registry + host-owned state projection (H9)** — the registry is the canonical list of devices (IDs, names, capability specs). Host-minted device ids are deterministic: `dev-<hex(SHA-256(installation_uuid, instance_id, local_id))>`; a plugin's re-registration on restart resurrects the same id, a reinstall (fresh `installation_uuid`) mints a new one. **Current state lives in a separate host-owned projection** (`DeviceStateStore`), keyed on `(device_id, capability)` — the registry never stored state past registration-time `initial_state` (review report H2). The projection is seeded from `initial_state` on `register-device`, updated automatically on every `publish-event` that carries a `state-changed` payload (under the same `publish_sequence` gate as `event_log.record`), and updated again on `execute-command` returns that carry `OkWithState` (the protocol treats that variant as authoritative state). Each entry carries a per-`(device, capability)` `revision`, a store-wide `global_revision`, host `received_ms` and plugin-supplied `observed_ms` (trust-separated, same shape as `event_log`), the supervisor `source_generation` at write time, and a `quality` marker (`fresh`/`stale`). Field updates **merge by key** (WIT `state-change.fields` is documented as *changes only*; replacing the whole vec would blow away previously-observed fields the plugin isn't currently reporting). Revision allocation and insertion linearize under one lock, so a reader observing `current_revision() = N` sees every entry with `global_revision ≤ N`. Lifecycle events reconcile the projection: `remove-device` flips the device's entries to `Stale`; `update-device` with a narrower `capabilities` list flips the dropped capabilities to `Stale`; `register-device` reconciles against the new capabilities list before seeding (so a stable device id resurrected from a prior life doesn't inherit omitted capabilities). When the owning instance stops (or a new supervisor life begins), every entry it owned flips to `stale` with a fresh `global_revision` so a delta poller catches the transition — consumers filter on `quality` instead of silently reading pre-restart values. Snapshot API: `GET /api/v1/devices/{id}/state` returns `{revision, capabilities}`; cursor catch-up: `GET /api/v1/devices/state/changes?since_revision=N&limit=M` returns entries with `global_revision > N`, ascending. Both gated on the new `devices:read` scope (distinct from `devices:list` which reads the registration snapshot, and `devices:command` which drives actuation).
