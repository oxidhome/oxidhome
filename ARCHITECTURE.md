...
8. **UI / API surface** — HTTP endpoints for clients to consume, currently split across a JSON/REST + WebSocket surface and a Connect RPC surface (which speaks Connect, gRPC, and gRPC-Web off the same handlers) on one shared listener. Plugin UIs (C6) load through a sandbox: `GET /api/v1/plugins/{plugin_id}/ui` (bearer + `plugins:ui`) returns a wrapper page whose only body is an `<iframe sandbox="allow-scripts allow-forms">` — deliberately no `allow-same-origin`, so the browser assigns each iframe a fresh opaque origin, distinct from the daemon's and from every other plugin's. The iframe targets `/api/v1/plugins/{plugin_id}/ui/frame?tk=<opaque>`; browsers won't propagate the parent page's `Authorization` header into an iframe subresource, so `/ui/frame` sits *outside* the bearer middleware and authenticates via a short-lived HMAC-signed ticket (5 min TTL, per-process secret, bound to `plugin_id`) that the wrapper mints and embeds in the src. The wrapper's inline broker JS establishes a fresh `MessageChannel` per iframe and transfers `port2` to it, so all subsequent host↔plugin IPC flows on a dedicated port — an origin-based `postMessage` filter would be useless here (opaque-origin senders all report `origin: "null"`), and the per-port binding makes cross-plugin dispatch impossible by construction. `plugin_id` is percent-encoded into the URL and HTML-escaped in every template slot to defend against a future validator relaxation that admits URL / HTML syntax.
